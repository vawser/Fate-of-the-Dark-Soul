RegisterTableGoal(GOAL_NPC_Pact_Sorcerer, "GOAL_NPC_Pact_Sorcerer")
REGISTER_GOAL_NO_SUB_GOAL(GOAL_NPC_Pact_Sorcerer, true)
...
Goal.Activate = function (self, ai, goal)
    ...
    ai:SetStringIndexedNumber("End_Backwalk_Dist", 8.0)     
    ...
    ai:SetStringIndexedNumber("Backstep_Retreat_Dist", 8.0)
    ...
    -- Act Distribution
    actChanceList[1] = 5 -- Light Attack
    actChanceList[2] = 5 -- Heavy Attack
    actChanceList[3] = 0 -- Kick
    actChanceList[4] = 0 -- Heavy Attack (Basic)
    actChanceList[5] = 0 -- Skill
    actChanceList[6] = 5 -- Running Attack
    actChanceList[7] = 100 -- Soul Arrow
    actChanceList[8] = 50 -- Great Heavy Soul Arrow
    actChanceList[9] = 25 -- Soul Spear
    ...
    -- Distance Modifiers
    if enemy_distance <= 3 then
        actChanceList[14] = 250 -- Backstep
    end
    
    if enemy_distance > 3 and enemy_distance <= 5 then
        actChanceList[11] = 250 -- Retreat
    end
    
    if enemy_distance >= 10 then
        actChanceList[10] = 1000 -- Approach
    end
    
    if enemy_distance >= 5 then
        actChanceList[1] = 0 -- Light Attack
        actChanceList[2] = 0 -- Heavy Attack
        actChanceList[6] = 0 -- Running Attack
    end
    ...
    -- Acts
    actFuncList[1] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act01) -- Light Attack
    actFuncList[2] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act02) -- Heavy Attack
    actFuncList[3] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act03) -- Kick
    actFuncList[4] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act04) -- Heavy Attack (Basic)
    actFuncList[5] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act05) -- Skill
    actFuncList[6] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act06) -- Running Attack
    actFuncList[7] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act07) -- Soul Arrow
    actFuncList[8] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act08) -- Great Heavy Soul Arrow
    actFuncList[9] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act09) -- Soul Spear
    
    actFuncList[10] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act10) -- Approach
    actFuncList[11] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act11) -- Retreat
    actFuncList[12] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act12) -- Strafe
    actFuncList[13] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act13) -- Forward Roll
    actFuncList[14] = REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_Act14) -- Backstep 
    
    Common_Battle_Activate(ai, goal, actChanceList, actFuncList, REGIST_FUNC(ai, goal, NPC_Pact_Sorcerer_ActAfter_AdjustSpace), actTblList)
    return 
end
...
function NPC_Pact_Sorcerer_Act01(self, ai, goal)
    -- Script
...
function NPC_Pact_Sorcerer_Act02(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Kick + Approach
function NPC_Pact_Sorcerer_Act03(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
function NPC_Pact_Sorcerer_Act04(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
-- Skill
function NPC_Pact_Sorcerer_Act05(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Running Attack
function NPC_Pact_Sorcerer_Act06(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Approach
function NPC_Pact_Sorcerer_Act10(self, ai, goal)
    if self:IsBothHandMode(TARGET_SELF) then
...
-- Retreat
function NPC_Pact_Sorcerer_Act11(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local duration = 1.8
    local animation = -1
...
-- Strafe
function NPC_Pact_Sorcerer_Act12(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    local duration = 1.8
...
-- Forward Roll
function NPC_Pact_Sorcerer_Act13(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Backstep
function NPC_Pact_Sorcerer_Act14(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Soul Arrow
function NPC_Pact_Sorcerer_Act07(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    
    local timer_duration = self:GetRandam_Int(0.5, 1)
    local wait_duration = 0.25
...
function NPC_Pact_Sorcerer_Act08(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Soul Spear
function NPC_Pact_Sorcerer_Act09(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Act After
-------------------------
function NPC_Pact_Sorcerer_ActAfter_AdjustSpace(self, ai, goal)
    return 
end
...
Goal.Interrupt = function (self, ai, goal)
    ...
    elseif ai:IsInterupt(INTERUPT_FindAttack) then
        goal:ClearSubGoal()
        if roll >= 75 then
            NPC_Pact_Sorcerer_Act12(ai, goal, paramTbl) -- Strafe
        else
            NPC_Pact_Sorcerer_Act14(ai, goal, paramTbl) -- Backstep
        end
    end

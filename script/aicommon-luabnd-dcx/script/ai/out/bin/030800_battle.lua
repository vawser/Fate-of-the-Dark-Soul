RegisterTableGoal(GOAL_NPC_Pact_Daredevil, "GOAL_NPC_Pact_Daredevil")
REGISTER_GOAL_NO_SUB_GOAL(GOAL_NPC_Pact_Daredevil, true)
...
Goal.Activate = function (self, ai, goal)
    ...
    actChanceList[2] = 30 -- Heavy Attack
    actChanceList[3] = 5 -- Kick
    ...
    -- Acts
    actFuncList[1] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act01) -- Light Attack
    actFuncList[2] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act02) -- Heavy Attack
    actFuncList[3] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act03) -- Kick
    actFuncList[4] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act04) -- Heavy Attack (Basic)
    actFuncList[5] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act05) -- Skill
    actFuncList[6] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act06) -- Running Attack
    
    actFuncList[10] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act10) -- Approach
    actFuncList[11] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act11) -- Retreat
    actFuncList[12] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act12) -- Strafe
    actFuncList[13] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act13) -- Forward Roll
    actFuncList[14] = REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_Act14) -- Backstep 
    
    Common_Battle_Activate(ai, goal, actChanceList, actFuncList, REGIST_FUNC(ai, goal, NPC_Pact_Daredevil_ActAfter_AdjustSpace), actTblList)
    return 
end
...
function NPC_Pact_Daredevil_Act01(self, ai, goal)
    -- Script
        ...
        if approachWeight_WithGuard > 0 then
            if enemy_Distance >= approachDistance_Maximum then
                ...
                if self:IsInsideTarget(TARGET_ENE_0, AI_DIR_TYPE_L, 180) then
                    ai:AddSubGoal(GOAL_COMMON_SidewayMove, 0.3, TARGET_ENE_0, 0, self:GetRandam_Int(75, 90), true, true, -1)
                else
                    ai:AddSubGoal(GOAL_COMMON_SidewayMove, 0.3, TARGET_ENE_0, 1, self:GetRandam_Int(75, 90), true, true, -1)
                end
            end
        end
    end
...
function NPC_Pact_Daredevil_Act02(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    ...
    if roll_b <= 50 and 0 < roll_c and max_attack_distance <= distance then
        if self:IsInsideTarget(TARGET_ENE_0, AI_DIR_TYPE_L, 180) then
            ai:AddSubGoal(GOAL_COMMON_SidewayMove, 0.3, TARGET_ENE_0, 0, self:GetRandam_Int(75, 90), true, true, -1) -- Move and Guard
        else
            ai:AddSubGoal(GOAL_COMMON_SidewayMove, 0.3, TARGET_ENE_0, 1, self:GetRandam_Int(75, 90), true, true, -1) -- Move and Guard
        end
    end
...
-- Kick + Approach
function NPC_Pact_Daredevil_Act03(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
function NPC_Pact_Daredevil_Act04(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
    local distance = self:GetDist(TARGET_ENE_0)
...
-- Skill
function NPC_Pact_Daredevil_Act05(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Running Attack
function NPC_Pact_Daredevil_Act06(self, ai, goal)
    local roll_a = self:GetRandam_Int(1, 100)
    local roll_b = self:GetRandam_Int(1, 100)
...
-- Approach
function NPC_Pact_Daredevil_Act10(self, ai, goal)
    if self:IsBothHandMode(TARGET_SELF) then
...
-- Retreat
function NPC_Pact_Daredevil_Act11(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local duration = 1.8
    local animation = -1
...
-- Strafe
function NPC_Pact_Daredevil_Act12(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
    local duration = 1.8
    local animation = -1
...
-- Forward Roll
function NPC_Pact_Daredevil_Act13(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Backstep
function NPC_Pact_Daredevil_Act14(self, ai, goal)
    local distance = self:GetDist(TARGET_ENE_0)
    local stamina = self:GetSp(TARGET_SELF)
...
-- Act After
-------------------------
function NPC_Pact_Daredevil_ActAfter_AdjustSpace(self, ai, goal)
    return 
end
...
Goal.Interrupt = function (self, ai, goal)
    ...
    elseif ai:IsInterupt(INTERUPT_FindAttack) then
        goal:ClearSubGoal()
        if roll >= 75 then
            NPC_Pact_Daredevil_Act12(ai, goal, paramTbl) -- Strafe
        else
            NPC_Pact_Daredevil_Act13(ai, goal, paramTbl) -- Forward Roll
        end
    end
